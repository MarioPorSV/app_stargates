...
function lista_aduana() {

    var url = base_url("index.php/AduanaController/lista_awb");
    $.get(url, function(data) 
    {
        $("#principal").html(data);
        var url = base_url("index.php/AduanaController/lista_awb_first");
        $.get(url, function(data) 
        {
            $("#awb_list").html(data);
        });
...
function consulta_warehouse() {
    var formData;
    var valido = "S";
    url_destino = "index.php/AduanaController/consulta_warehouse";
...
function verificar_warehouse() {
    ...
    if ($("#estatus").val() == "") {
        ...
        document.getElementById("msg_alert").innerHTML =
            "Debe seleccionar un estatus, para poder continuar";
        cerrar_alert();
        return false;
    }
    if ($("#search").val() == "") {
        ...
        document.getElementById("msg_alert").innerHTML =
            "Debe ingresar un número de Warehouse, para poder continuar";
        cerrar_alert();
        return false;
    }
    ...
    var url = base_url(
        "index.php/AduanaController/verificar_warehouse/" + warehouse
    );
    ...
    $.getJSON(url, {
            ...
            url_destino = "index.php/AduanaController/guardar_estatus/";
...
function enviar_email(id) {
    ...
    $.ajax({
        ...
        success: function(result) {
            $("#message").html(
                '<div class="alert alert-success alertgreen "><button type="button" class="close">x</button><strong>Correo electrónico ha sido enviado correctamente</strong></div>'
            );
...
function upload_fssile() {
    var url = base_url("index.php/AduanaController/upload_file/");
    $.ajax({
        url: url,
        data: $("form").serialize(),
        type: "POST",
...
function upload_file_facturacion() {
    var formData;
    var url = base_url("index.php/AduanaController/upload_file_facturacion/");
    formData = new FormData($(".upwh")[0]);
    $.ajax({
        url: url,
        type: "POST",
...
function listado_warehouse() {
    var url = base_url("index.php/AduanaController/listado_warehouse");
    $.get(url, function(data) {
...
function consulta_wh() {
    ...
    var url = base_url("index.php/AduanaController/consulta_warehouse/" + campo + "/" + args);
    $.get(url, function(data) {
        $("#table_wh").html(data);
...
function manifiesto_listado() {
    ...
    var formData;
    url_destino = "index.php/AduanaController/manifiesto_listado" + "/" + $("#input_filtro").val() + "/" + opc;
...
function consulta_awb() {
    ...
    var url = base_url("index.php/AduanaController/consulta_awb/" + inicio + "/" + fin + "/" + opcion + "/" + buscar);
    $.get(url, function(data) {
        $("#awb_list").html(data);
    });
}
...
function lista_warehouse(id, master,correla) {
  //  alert(id+" "+master)
   //alert("miercoles");
    /** busca los detalles (warehouse de una master) tabla manifiesto */
    master_number =  master;
    //master_number = "AWB: " + master;
    var url = base_url("index.php/AduanaController/manifiesto/" + id + "/" + master + "/" + correla);
...
function lista_warehouse_confirma(id, master, correla) {
  //  alert(id+" "+master)
   //alert("miercoles");
    ...
    master_number =  master;
    //master_number = "AWB: " + master;
    var url = base_url("index.php/AduanaController/manifiesto_confirma/" + id + "/" + master + "/" + correla);
...
function lista_warehouseexport() {
    ...
    var master = 0;
	var url = base_url(
		"index.php/AduanaController/manifiesto_export/" + id + "/" + master
	);
	$.get(url, function (data) {
		$("#principal").html(data);
		//$("#master-number").val(master_number);
...
function lista_warehouse1() {
	...
	//master_number = "AWB: " + master;
  //  console.log("el  id del  manifiestos es " + id);

	var url = base_url(
		"index.php/AduanaController/generar_manifiesto/" + id + "/"+ referencia
	);
...
function lista_pendientes_confirmar() {
    ...
    var url = base_url("index.php/AduanaController/lista_pendientes_confirmar");
    $.get(url, function(data) {
        $("#principal").html(data);
        $("#opcion").val(0);
        $('td:nth-child(20)').hide();
        //    $('td:nth-child(19),th:nth-child(19)').hide(); //para ocultar encabezado y detalle
...
function lista_confirmados() {
     ...
     var url = base_url("index.php/AduanaController/lista_awb_confirma");
    $.get(url, function(data) {
        $("#principal").html(data);
        var url = base_url("index.php/AduanaController/lista_awb_first_confirma");
        $.get(url, function(data) {
            $("#awb_list").html(data);
        });
    ...
    //var url = base_url("index.php/AduanaController/lista_confirmados");
   // $.get(url, function(data) {
    //    $("#principal").html(data);
     //   $("#opcion").val(1);
      //  $('td:nth-child(19)').hide(); //oculata columna de conversacion
    //    document.getElementById("titulo").innerHTML = "Paquetes pendientes de facturar";
   // });
}
...
function crear_guia_express() {
    ...
    //crea la guia araves de la api
    id = $('#order_number').val();

    var formData;

    url_destino = "index.php/AduanaController/obtener_guia/" + id;
...
function update_tracking_id(id, recolecta, guia) {
    ...
    var url = base_url("index.php/AduanaController/update_tracking_id/" + id + "/" + recolecta + "/" + guia);
...
function catalogos_pais_new() {
    var url = base_url(
        "index.php/AduanaController/lista_departamentos"
    );
    ...
    $.get(url, function(data) {
        ...
        $("#departamento").change(function() {
            //  $('#nepto').val($("#departamento option:selected").text()); //muestra el nombre
            $('#depto').val($("#departamento").val());
            // $('#nservi').val($("#servicio option:selected").text());
            var url = base_url(
                "index.php/AduanaController/lista_municipios/" + $('#departamento').val()
            );
...
function get_municipio(id) {
    var url = base_url(
        "index.php/AduanaController/lista_municipios/" + id
    );
...
function lista_referencia(id, master, ref) {
    ...
    var url = base_url("index.php/AduanaController/lista_referencia/" + id + "/" + ref);
...
function guardar_referencia() {
    ...
    url_destino = "index.php/AduanaController/guardar_referencia/";
...
function asignar_guia() {
    ...
    url_destino = "index.php/AduanaController/asignar_guia/";
...
function mostrar_conversacion(id, tracking, consignee) {
    ...
    var url = base_url(
        "index.php/AduanaController/consulta_conversacion/" + id
    );
...
function guardar_comentario() {
    ...
    url_destino = "index.php/AduanaController/guardar_comentario/";
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (aceptado == 1) {
                var url = base_url(
                    "index.php/AduanaController/update_aceptado/" + id
                );
...
function guardar_item_multi() {
    ...
    url_destino = "index.php/AduanaController/guardar_item_multi/" + id + "/" + cantidad;
    formData = new FormData($(".add_comenta")[0]);
...
function asignar_partida(id, partida) {
    ...
    var url = base_url("index.php/AduanaController/asignar_partida/" + id + "/" + partida);
...
function pdf_wh(id,master){
    ...
    var url = base_url("index.php/AduanaController/pdf_wh/" + id + "/" + master);
...
{
    var url = base_url("index.php/AduanaController/pdf_wh2_bk/" + id + "/" + master);
    let name = master + '.pdf';
    $.get(url, function(data) 
    {
        $("#ContPDF").html(data);
...
{
    var url = base_url("index.php/AduanaController/pdf_wh2/" + id + "/" + master);
    $.get(url, function(data) 
    {
        var datos = JSON.parse(data);
...
function lista_servicios_express2(id) {
    //console.log(id);
    var url = base_url("index.php/AduanaController/departamento");
    $.get(url, function(data) {
        //console.log(data);
        //alert(data);
...
function aplicar_estatus(guia) {
	...
	guia = $("#guia").val();
	//	alert(guia);
	var url = base_url("index.php/AduanaController/aplicar_estatus/" + guia);
	$.get(url, function (data) {
		$("#principal").html(data);
		if (!guia == 0) {
...
function update_info() {
    ...
    url_destino = "index.php/AduanaController/update_info"  ;
...
function upload_fileDUI() {
	...
	var formData;
	var url = base_url(
		"index.php/AduanaController/upload_fileDUI/" + idpreclasificacion
	);
	formData = new FormData($(".upwh")[0]);
	$.ajax({
		url: url,
...
function upload_file1() {
	...
	var formData;
	var url = base_url(
		"index.php/AduanaController/upload_fileWH/" + idpreclasificacion
	);
	formData = new FormData($(".upwh")[0]);
	$.ajax({
		url: url,
...
function verificar_lm(){
    id=$("#id-master").val();
     ...
     alertify
        .confirm(
            "¿Esta seguro de ejecutar este proceso?",
            function() {
               
                var url = base_url(
                    "index.php/AduanaController/verificar_lm/" + id
                );
        ...
        .set({
            title: "Verificar departamentos y municipios"
        })
        .set({
            labels: {
                ok: "Aceptar",
...
{  
	...
	var formData;
	var url = base_url("index.php/AduanaController/reemplazar_tracking/" + id + "/" + tracking_number + "/" + tracking_replace);
...
{
    var formData;
    url_destino = "index.php/AduanaController/historial/" + tracking_number;
...
{
	var url = base_url("index.php/AduanaController/cargar_referencia/");
	$.get(url, function (data) 
    {
		//$("#principal").html(data);
...
function upload_file_referencia()
{
    ...
    //alert(idpreclasificacion);
    var url = base_url("index.php/AduanaController/upload_file_referencia/" + idpreclasificacion);
...
{
    var url = base_url("index.php/AduanaController/reporte_manifiesto/" + idpreclasificacion);
    $.get(url, function (data) 
    {
        $("#principal").html(data);
    });
...
function reporte_guias(idpreclasificacion)
{
  ...
  var url = base_url("index.php/AduanaController/reporte_guias/" + idpreclasificacion);
...
{
	var url = base_url("index.php/AduanaController/lista_sobrantes/");
	$.get(url, function (data) 
    {
...
function upload_file_sobrante()
{
    ...
    //alert(idpreclasificacion);
    var url = base_url("index.php/AduanaController/upload_file_sobrante/" + idpreclasificacion);
...
{
    var url = base_url("index.php/AduanaController/asignar_ref/" + idpreclasificacion);
	$.get(url, function (data) 
    {
		$("#principal").html(data); 
...
{ 
    ...
    url_destino = "index.php/AduanaController/update_ref_bolsa/" + referencia + "/" + bolsas;
...
{
    var url = base_url("index.php/AduanaController/referencia_upd/" + tracking_number);
	$.get(url, function(data) 
    {
...
{ 
   ...
   // var tracking_number     = $("#tracking_number").val(); 
	var formData;
	var url = base_url("index.php/AduanaController/add_reem_referencia/"  + tracking_number + "/" + referencia);
